    ...
    return  (
        ...
        <div className="col-25">
          <div className="container-checkout-two">
            ...
            {cartSaver.map(p => <p key={p.id}><button type="button" className="btn btn-outline-primary">{p.title} {p.quantity === 1 ? "" : `(${p.quantity})`}</button> <span className="price">${p.totalPrice}</span></p>)}
            <hr/>
